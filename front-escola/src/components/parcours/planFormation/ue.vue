    ...
    <div class="layout-padding" v-if="isReady">
      ...
      <q-modal v-model="modalUpdateUE" :content-css="{minWidth: '40vw', minHeight: '40vh'}">
        <div class="layout-padding">
          <q-input v-model="editedUE.libelle" float-label="Libelle" />
          <q-input v-model="editedUE.code" float-label="Code" />
          <q-input v-model="editedUE.abreviation" float-label="Abréviation" />
          <q-select
            v-model="editedUE.isCompensable"
            float-label="Compensable"
            :options="is_compensableOptions"
          />
          <q-select
            v-model="editedUE.typeUe"
            float-label="Type UE"
            :options="typeUe"
          />
          <q-select
            :options="utilisateurs"
            v-model="editedUE.responsableUe"
            chips
  ...
  export default {
    ...
    data () {
      return {
        newEc: {
          ...
          noteEliminatoire: '',
          ue: {
            id: this.$store.getters['planformation/selectedUE'].id
          }
        },
        allEc: [],
    ...
    methods: {
        ...
        this.$store.dispatch('ec/addEc', ec)
          .then((res) => {
            ...
            if (this.newEc.typeEc === 2) {
              console.log('LG 377 :' + this.ue.id)
              this.$store.dispatch('ec/getIdecOptOb', this.ue.id)
                .then((resid) => {
                  let tabIdecopt = []
                  console.log('LG 381 :' + resid)
                  map(resid, (ecselect) => {
                    let idecopt = {
                      id: ecselect.id
                    }
      ...
      checkEcCollection () {
        ...
        this.$store.dispatch('ec/getAllEcOfUe', ueId)
          .then((ecs) => {
            this.allEc = ecs
          })
          .catch((err) => {
      ...
      updateUe () {
        let params = {
          ...this.editedUE,
          isCompensable: this.editedUE.isCompensable,
          idTypeUe: this.editedUE.typeUe,
          responsableUe: this.getSelected(this.editedUE.responsableUe),
          enseignant: this.getSelected(this.editedUE.enseignant)
        }
      ...
      updateEc () {
        let ec = {
          ...this.selectedEC,
          typeEc: this.getSelected(this.selectedEC.typeEc),
