            ...
            <q-card >
              ...
              <q-card-main style="margin-top: 0; padding: 20px 50px;">
                <div class="_image-upload">
                  <div v-ripple="" style="overflow: hidden;cursor: pointer;padding-bottom: 0;margin-bottom: 0;font-size: 0;width: 55%;" class="relative-position flex flex-center items-center">
  ...
  export default {
    ...
    computed: {
      allRequiredFieldStepOneFill: function () {
        return this.form.sexe !== '' && this.form.nom !== '' && this.form.dateNais !== '' && this.form.lieuNais !== '' && this.form.nationalite !== '' && this.form.adresse !== '' && this.form.tel !== '' && this.form.numBacc !== '' && this.form.anneeBacc !== '' && this.form.provinceBacc !== '' && this.form.bacc
      },
      allRequiredFieldStepThreeFill: function () {
        return this.form.email !== '' && this.form.password !== '' && this.form.passwordRepeat
      },
      allRequiredFieldStepTwoFill: function () {
        return this.form.matricule !== '' && this.annee !== '' && this.mention !== '' && this.parcours !== '' && this.semestre !== []
      }
    },
    watch: {
      annee: function (val) {
    ...
    methods: {
          ...
          this.$store.dispatch('getFonction')
            .then((fonction) => {
              ...
              let data = {
                ...
                codeRedoublement: this.codeRedoublement
              }
              this.$store.dispatch('inscription/findEtudiantImatricule', this.form.matricule)
                .then((retour) => {
                  if (retour.length >= 1) {
                    Loading.hide()
                    this.$alert.error('Erreur lors de l\'inscription de l\'étudiant. Le numéro imatricule ' + this.form.matricule + ' existe déjà.')
                  }
                  else {
                    this.$store.dispatch('inscription/inscrireEtudiant', data)
                      .then((res) => {
                        ...
                        this.$store.dispatch('utilisateur/envoimailbymail', this.form.email)
                          .then((resmail) => {
      ...
      goToStep2 () {
        if (this.$refs.stepper.step === 'etat_civil') {
          ...
          if (this.allRequiredFieldStepThreeFill) {
            this.$store.dispatch('inscription/findPersonneByEmail', this.form.email).then((res) => {
              if (res.length >= 1) {
                this.$alert.error('veuillez changer votre l\'adresse e-mail car cette dernier existe déjà.')
              }
              else {
                this.$refs.stepper.next()
              }
