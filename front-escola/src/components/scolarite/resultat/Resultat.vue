<template>
  ...
  <div class="layout-padding">
    ...
    <div class="row layout-view content-center">
      ...
      <div v-else-if="compensable == true" class="content-center" style="margin: 0 auto">
        <table v-if="notes.length > 0" class="q-table cell-separator loose highlight _table-resultat">
          <thead>
            <tr>
              ...
              <th class="text-center" :colspan="4">RESUMÉ</th>
            </tr>
            <tr>
              <template v-for="noteUe in notes[0].noteParUe">
          ...
          <tbody>
            <tr v-for="note in notes" :key="note.etudiantInfo.numeroMatricule">
              ...
              <td class="text-center text-bold">{{ note.moyenneSemestre }}</td>
              <td class="text-center text-bold">
                <q-btn color="secondary" @click="compenser(note)">
    ...
    <q-modal ref="compenserModal" class="_compenserModal" v-model="openCompenser"
        ...
        <div slot="header" class="text-grey-8" style="border-bottom: 1px solid lightgrey">
          <div class="">
            <div class="col-sm-12 layout-padding ">
              <div style="margin: 0;font-size: 24px" class="text-negative">Compensation</div>
            </div>
          </div>
          <q-btn flat round small @click="$refs.compenserModal.close()" style="position: absolute;top: 1em;right: 1em">
            <q-icon name="close" />
          </q-btn>
        </div>
        <div class="layout-padding">
          <div class="text-grey text-italic">Voulez vous vraiment Valider le {{ semestreACompenser }} de {{ etudiantACompenserNom }} {{ etudiantACompenserPrenom }} par Compensation ?</div>
          <div class="text-grey text-italic">Liste UE à compenser :</div>
          <q-input
            v-model="description"
            color="tertiary"
            type="textarea"
            float-label="Remarque"
            :min-rows="5"
          />
        </div>
  ...
  export default {
    ...
    data () {
      return {
        ...
        description: '',
        etudiantAcompenser: '',
        etudiantACompenserNom: '',
        etudiantACompenserPrenom: '',
        semestreACompenser: '',
        idUeAcompenser: [],
        choice: '',
        annee: '',
    ...
    methods: {
      ...
      compenser (selection) {
        ...
        this.idUeAcompenser = []
        this.etudiantAcompenser = selection
        this.semestreACompenser = selection.semestre.libelle
        this.allUe = selection.noteParUe
        this.etudiantACompenserNom = selection.etudiantInfo.nom
        this.etudiantACompenserPrenom = selection.etudiantInfo.prenom
        map(this.allUe, (ue) => {
          console.log(ue)
          if (ue.status === 'NON_VALIDE') {
            this.idUeAcompenser.push(ue.ueInfo.id)
          }
        })
        this.openCompenser = true
        console.log(this.idUeAcompenser)
      },
      compenserEtudiant () {
        let params = {
          description: this.description,
          id_annee_univ: this.etudiantAcompenser.semestre.semestrePK.idAnneeUniversitaire,
          id_etudiant: this.etudiantAcompenser.etudiantInfo.id,
          id_semestre: this.etudiantAcompenser.semestre.semestrePK.id,
          id_session: this.session,
          idue: this.idUeAcompenser
        }
        this.$store.dispatch('compensation/addCompensation', params)
          .then((res) => {
            this.openCompenser = false
            let alert = Alert.create({
              html: 'Le semestre de ' + this.etudiantACompenserNom + '  ' + this.etudiantACompenserPrenom + ' est Validé par Compensation',
              enter: 'bounceInLeft',
          ...
          .catch((err) => {
            if (err) {
              console.log(err)
              let alert = Alert.create({
                html: 'Le semestre ne peut pas être validé par Compensation',
                enter: 'bounceInLeft',
                leave: 'bounceOutRight',
                color: 'red',
                position: 'top-right',
